...
export class EditorStateService {
    ...
    private _map_width = signal<number>(100);
    private _map_height = signal<number>(100);
    private _embeded = signal<boolean>(false);
    private _map_regions = signal<MapRegion[]>([]);
    private _active_region = signal<MapRegion>(null);
    ...
    public readonly regions = this._map_regions.asReadonly();
    ...
    public setWidth(w: number) {
        this._map_width.set(w);
    }

    public setHeight(h: number) {
        this._map_height.set(h);
    }

    public setRatio(r: number) {
        this._map_height.set(Math.floor(this._map_width() * r * 100) / 100);
    ...
    private _cleanUpPoint() {
        if (this._move) {
            ...
            const updated_region = this._map_regions().find(
                (_) => _.id === active_region.id,
            );
            this._active_region.set(updated_region);
            this._move = false;
        }
    }
    ...
    public newRegion() {
        const regions = this._map_regions();
        ...
        this._map_regions.set([...regions, new_region]);
    ...
    public removeRegion(region: MapRegion) {
        ...
        const active_region = this._active_region();
        this._map_regions.set(list);
        if (active_region?.id === region.id) {
            this._active_region.set(list[0] || null);
        }
    }
    ...
    public async saveMetadata() {
        ...
        const data = {
            url: this._use_url() || this._map_url(),
            width: 0,
            height: 0,
            areas: this._map_regions().map(formatRegion),
        };
        if (embeded) {
            await sendMessage({
                type: 'backoffice',
    ...
    private async loadRegionData() {
        ...
        this._map_regions.set(map_data.areas);
    }
