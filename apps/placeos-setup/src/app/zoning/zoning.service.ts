...
export class ZonesService {
    ...
    public setSelected(id: string, state: boolean) {
        ...
        if (id === '*') {
            this._selected.next(
                !state ? [] : this._zone_list.getValue().map((_) => _.id)
            );
            return;
        }
    ...
    public setZone(zone: Zone) {
        ...
        this._zone_list.next([
            ...this._zone_list.getValue().filter((_) => _.id !== zone.id),
            zone,
        ]);
        this._store();
    }
    ...
    public async removeSelected() {
        ...
        if (list.length === 1) {
            return this.removeZone(
                this._zone_list.getValue().find((_) => _.id === list[0])
            );
        }
        const { close, reason } = await openConfirmModal(
            {
    ...
    private _store() {
        localStorage.setItem(
            'PLACEOS_BUILD.Zones',
            JSON.stringify(this._zone_list.getValue())
        );
    }
}
