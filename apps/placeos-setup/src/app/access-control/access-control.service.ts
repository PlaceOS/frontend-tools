...
export class AccessControlService {
    ...
    public setAccessControl(accesscontrol: AccessControl) {
        ...
        this._access_control_list.next([...this._access_control_list.getValue().filter(_ => _.id !== accesscontrol.id), accesscontrol]);
        this._store();
    }
    ...
    public async removeAccessControl(item: AccessControl) {
        ...
        this._access_control_list.next(this._access_control_list.getValue().filter(_ => _.id !== item.id));
        this._store();
        close();
    }
    ...
    public async removeSelected() {
        ...
        if (list.length === 1) {
            return this.removeAccessControl(this._access_control_list.getValue().find(_ => _.id === list[0]));
        }
        const { close, reason } = await openConfirmModal({
        ...
        if (reason !== 'done') return;
        this._access_control_list.next(this._access_control_list.getValue().filter(_ => !list.find(id => _.id === id)));
        this._selected.next([]);
        this._store();
        close();
    }
    ...
    private _load() {
        const data = JSON.parse(localStorage.getItem('PLACEOS_BUILD.AccessControls') || '[]');
        this._access_control_list.next(data);
    }

    private _store() {
        localStorage.setItem('PLACEOS_BUILD.AccessControls', JSON.stringify(this._access_control_list.getValue()));
    }
}